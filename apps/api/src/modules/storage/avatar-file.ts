import { BadRequestException } from "@nestjs/common";

export const MAX_AVATAR_FILE_SIZE = 5 * 1024 * 1024;

export interface UploadedAvatarFile {
	buffer: Buffer;
	mimetype: string;
	originalname: string;
	size: number;
}

export interface ValidatedAvatarFile {
	contentType: "image/gif" | "image/jpeg" | "image/png" | "image/webp";
	extension: ".gif" | ".jpg" | ".png" | ".webp";
}

const AVATAR_FORMATS: ReadonlyArray<
	ValidatedAvatarFile & { matches: (buffer: Buffer) => boolean }
> = [
	{
		contentType: "image/jpeg",
		extension: ".jpg",
		matches: (buffer) =>
			buffer.length >= 3 &&
			buffer[0] === 0xff &&
			buffer[1] === 0xd8 &&
			buffer[2] === 0xff,
	},
	{
		contentType: "image/png",
		extension: ".png",
		matches: (buffer) =>
			buffer.length >= 8 &&
			buffer
				.subarray(0, 8)
				.equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])),
	},
	{
		contentType: "image/gif",
		extension: ".gif",
		matches: (buffer) => {
			const signature = buffer.subarray(0, 6).toString("ascii");
			return signature === "GIF87a" || signature === "GIF89a";
		},
	},
	{
		contentType: "image/webp",
		extension: ".webp",
		matches: (buffer) =>
			buffer.length >= 12 &&
			buffer.subarray(0, 4).toString("ascii") === "RIFF" &&
			buffer.subarray(8, 12).toString("ascii") === "WEBP",
	},
];

export function validateAvatarFile(
	file: UploadedAvatarFile | undefined,
): ValidatedAvatarFile {
	if (!file || file.buffer.length === 0) {
		throw new BadRequestException("No file provided");
	}

	if (
		file.size > MAX_AVATAR_FILE_SIZE ||
		file.buffer.length > MAX_AVATAR_FILE_SIZE
	) {
		throw new BadRequestException("File too large (max 5MB)");
	}

	const detectedFormat = AVATAR_FORMATS.find(({ matches }) =>
		matches(file.buffer),
	);
	if (!detectedFormat || detectedFormat.contentType !== file.mimetype) {
		throw new BadRequestException(
			"Invalid file type. Allowed: JPEG, PNG, GIF, WebP",
		);
	}

	return {
		contentType: detectedFormat.contentType,
		extension: detectedFormat.extension,
	};
}
